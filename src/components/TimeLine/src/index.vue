  ...
  <div
    ...
    <div
      ...
      <WindowListItem
        ...
        @click="toggleActive(index)"></WindowListItem>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    setInitData() {
      ...
      this.currentZoomIndex =
        this.initZoomIndex >= 0 && this.initZoomIndex < ZOOM.length ? this.initZoomIndex : 5;
      // 初始当前时间
      this.startTimestamp =
        (this.initTime
          ? typeof this.initTime === "number"
            ? this.initTime
            : new Date(this.initTime).getTime()
    ...
    getCurrentDayAndPreDay() {
      const nextDay = dayjs(
        dayjs(this.initTime).add(1, "day").format("YYYY-MM-DD 00:00:00")
      ).valueOf();

      const currentDay = dayjs(dayjs(this.initTime).format("YYYY-MM-DD 00:00:00")).valueOf();
    ...
    init() {
      ...
      this.width = width;
      this.height = this.windowList.length > 1 ? this.baseTimeLineHeight : height;
      this.$refs.canvas.width = this.width;
      this.$refs.canvas.height = this.height;
      this.ctx = this.$refs.canvas.getContext("2d");
      this.currentTime = dayjs(this.initTime).valueOf();
    ...
    drawCurrentTimeLine() {
      ...
      // 画当前时间的线，如果不在时间范围内则不画
      const { nextDay } = this.getCurrentDayAndPreDay();
      // 取最小的结束值，防止超出
      const endTimeStamp = Math.min(this.startTimestamp + this.totalMS, nextDay);

      if (this.currentTime < this.startTimestamp || this.currentTime > endTimeStamp) return;

      const currentTimeMs = this.currentTime - this.startTimestamp;
      // 一共可以绘制的格数
      let gridNum = ZOOM[this.currentZoomIndex] / this.ACT_ZOOM_HOUR_GRID[this.currentZoomIndex];
      // 一格多少毫秒
      let msPerGrid = this.ACT_ZOOM_HOUR_GRID[this.currentZoomIndex] * ONE_HOUR_STAMP;
      // 每格间距，一格多少像素宽
      let pxPerGrid = this.width / gridNum;
      // 算出距离开始时间的长度
      const x = Math.ceil((currentTimeMs / msPerGrid) * pxPerGrid);
      let { height } = this.$refs.timeLineContainer.getBoundingClientRect();
      this.drawLine(x, 0, x, height, width, color);
    },
    /**
     * @Author: 王林25
    ...
    addGraduations() {
      ...
      for (let i = 0; i < gridNum; i++) {
        ...
        if (this.showDateAtZero && date.getHours() === 0 && date.getMinutes() === 0) {
          h =
            this.height *
            (this.lineHeightRatio.date === undefined ? 0.3 : this.lineHeightRatio.date);
          ...
          h =
            this.height *
            (this.lineHeightRatio.time === undefined ? 0.2 : this.lineHeightRatio.time);
          this.ctx.fillStyle = this.textColor;
          this.ctx.fillText(this.graduationTitle(graduationTime), x - 13, h + 15);
        } else {
          // 不显示时间的线段
          h =
            this.height *
            (this.lineHeightRatio.none === undefined ? 0.1 : this.lineHeightRatio.none);
        }
    ...
    drawTimeSegments(callback, path) {
      ...
      this.timeSegments.forEach((item) => {
        if (item.beginTime <= this.startTimestamp + this.totalMS) {
    ...
    drag(x) {
      ...
      // 判断是否超出限制范围
      let hfms = this.totalMS / 2;
      let _newStartTimestamp = this.mousedownCacheStartTimestamp - Math.round(diffX / PX_PER_MS);
    ...
    hoverShow(x, noDraw) {
      ...
      let h =
        this.height * (this.lineHeightRatio.hover === undefined ? 0.3 : this.lineHeightRatio.hover);
      this.drawLine(x, 0, x, h, 1, this.lineColor);
     ...
     */
    setZoom(index) {
      this.currentZoomIndex = index >= 0 && index < ZOOM.length ? index : 5;
      this.clearCanvas(this.width, this.height);
      this.startTimestamp = this.currentTime - this.totalMS / 2; // 当前时间-新的时间范围的一半
      this.draw();
    },
